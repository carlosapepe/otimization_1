...
def printOutput(x, actors, optV, n_nodes, fim, inicio):
    ...
    print('duracao: %fs' % (fim - inicio))
    print('numero de nós: %d' % n_nodes)
...
def bound2(actor, n_character, cast):
    global optV
    # global optX
    cost = 0
    # print('cast ', cast)
...
#         return True
def bound1(cast,n_actor,n_character,level):
#def bound2(cast,n_character,level):
...
def branch1(cast, level, n_character, n_actor, actor, n_group):
    ...
    global optV
    global optX
    if bound1(cast,n_actor,n_character,level):
        return
    ...
    # 0
    aux = cast[:]
    if len(aux) < n_actor and bound2(actor, n_character, cast):
        aux.append(0)
        n_nodes+=1
...
def main(argc, argv):
    ...
    lower = sorted(actors, key=lambda x: x.cost, reverse=False)[:n_character]
    auxv = sum(l.cost for l in lower)
    optV = auxv
